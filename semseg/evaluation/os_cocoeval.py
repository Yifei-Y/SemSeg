...
class OpensetCOCOEval(COCOeval):
    ...
    def accumulate(self, p = None):
        ...
        for k, k0 in enumerate(k_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    okfp_sum = np.sum(okfps, axis=1).astype(dtype=np.float64)
                    ufp_sum = np.cumsum(ufps, axis=1).astype(dtype=np.float64)
        ...
        for a, a0 in enumerate(a_list):
            ...
            for m, maxDet in enumerate(m_list):
                ...
                k_det_as_unk_fp_sum = np.cumsum(k_det_as_unk_fps, axis=1).astype(dtype=np.float64)
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            stats[14] = _num_unk_det_as_known(iouThr=.5, areaRng='all', maxDets=100)
